...
def ESI(weights:tuple):
    data = pd.read_csv('Complete_Dataset_updated.csv')
    w1, w2, w3 = weights
    ESI_acutal = data['Environmental Suitability Index']
    ESI_pred = w1*data['Rainfall'] + w2*data['TEMP_MAX'] + w3*data['EVI']
    return mean_squared_error(ESI_acutal, ESI_pred)
